...
class AggregateRankingCalculator():
    ...
    def get_matchup(self, player):
        ...
        for p in player:
            points.append(0)
            if (p, "psn") not in self.all_users and (p, "xbl") not in self.all_users and (
                    p, "switch") not in self.all_users:
                print("Error: %s is not in the player database. Exiting program." % p)
                return None
...
def main():
    if len(sys.argv) < 3:
        print("correct usage: python %s [p1] [p2] [p3] etc... " % sys.argv[0])
        exit()
    ...
    af_calc = AggregateRankingCalculator(players)
